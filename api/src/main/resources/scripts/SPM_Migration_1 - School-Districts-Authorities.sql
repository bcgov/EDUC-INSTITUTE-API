...
SELECT
    ...
    CASE
        WHEN dist_mast.DISTNO = '102' THEN 'PSI'
        WHEN dist_mast.DISTNO = '098' THEN 'YUKON'
        WHEN dist_mast.DISTNO = '103' THEN 'OFFSHORE'
    ELSE (SELECT dist_reg.DISTRICT_REGION_CODE from DISTRICT_REGION_CODE dist_reg where TRIM(dist_mast.DIST_AREA_CODE) = dist_reg.LEGACY_CODE)
    END as DISTRICT_REGION_CODE,
...
SELECT
    ...
    CASE WHEN auth_mast.SCHOOL_AUTHORITY_NAME_LONG = ' ' then TRIM(INITCAP(auth_mast.SCHOOL_AUTHORITY_NAME)) ELSE TRIM(INITCAP(auth_mast.SCHOOL_AUTHORITY_NAME_LONG)) END as DISPLAY_NAME,
...
SELECT
    ...
    CASE
        WHEN schl_mast.SCHOOL_CATEGORY_CODE = '02' AND REGEXP_LIKE(schl_mast.SCHLNO, '^97[0-9]*') THEN 'INDP_FNS'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'POLSP' AND schl_mast.SCHOOL_CATEGORY_CODE = '01' THEN 'PUBLIC'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'POLSP' AND schl_mast.SCHOOL_CATEGORY_CODE != '01' THEN 'YUKON'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'DOLS' THEN 'PUBLIC'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'POLSI' THEN 'INDEPEND'
    ELSE (SELECT schl_cat.SCHOOL_CATEGORY_CODE from SCHOOL_CATEGORY_CODE schl_cat where TRIM(schl_mast.SCHOOL_CATEGORY_CODE) = schl_cat.LEGACY_CODE)
    END as SCHOOL_CATEGORY_CODE,
    CASE
        WHEN schl_mast.OL_SCHOOL_TYPE = 'POLSP' THEN 'DIST_LEARN'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'DOLS' THEN 'DISTONLINE'
        WHEN schl_mast.OL_SCHOOL_TYPE = 'POLSI' THEN 'DIST_LEARN'
    ELSE (SELECT fac_type.FACILITY_TYPE_CODE from FACILITY_TYPE_CODE fac_type where TRIM(schl_mast.FACILITY_TYPE_CODE) = fac_type.LEGACY_CODE)
...
UPDATE SCHOOL SET FACILITY_TYPE_CODE = 'POST_SEC'
